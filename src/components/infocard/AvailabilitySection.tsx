...
export default function AvailabilitySection({
  ...
  function makeAvailEle(dow: number) {
    const availDay = availability[String(dow)];

    if (!availDay) {
      return [
        <div
          className="bg-collection-1-highlight mb-0 ml-2 mr-2 mt-3 rounded-[4px] opacity-80"
          key="bad"
        >
          <p className="ml-2">No time info</p>
        </div>,
      ];
    }
    ...
    return availDay.map((s) => (
      <div
        className="bg-blur-sm bg-collection-1-highlight mb-0 ml-2 mr-2 mt-0 rounded-[4px] opacity-80"
        key={s[0][0] + s[1]}
      >
        {' '}
  ...
  return (
    <div className="bg-collection-1-background ml-1 mr-1 rounded-[8px] pb-1 pt-1">
      <p className="ml-2 mt-2">
        {classNow ? classNow + ' is now.' : 'No class now!'}
      </p>
      <Tabs
