...
export default function BuildingSearchResults({
  ...
  const filteredRooms: RoomWithOrdinal[] = useMemo(() => {
    ...
    let lDistCache = new Map()
    // Query for another building
    let roomsList = building.floors.flatMap((floor: Floor) => (
      floorMap[`${building.code}-${floor.name}`]
        ?.rooms
        .filter((room: Room) => {
          const fullName = fullRoomName(room, building)
          const fullCodeName = fullRoomName(room, building, true)
          const a = levenDist(simplifiedQuery.toLowerCase(), fullName.toLowerCase())
          const b = levenDist(simplifiedQuery.toLowerCase(), fullCodeName.toLowerCase())
          const c = !!room.alias && levenDist(simplifiedQuery.toLowerCase(), room.alias.toLowerCase())
          const d = !!room.type && levenDist(simplifiedQuery.toLowerCase(), room.type.toLowerCase())
    ...
    if(userPosition) 
      roomsList.sort((a, b)=> distance(a.labelPosition, userPosition)-distance(b.labelPosition, userPosition))
  ...
  if (filteredRooms.length == 0 
    && levenDist(building.name.substring(0, ogQuery.length).toLowerCase(), ogQuery.toLowerCase()) > 2 
    && levenDist(building.code.substring(0, ogQuery.length).toLowerCase(), ogQuery.toLowerCase()) > 2) {
      return null;
    }
