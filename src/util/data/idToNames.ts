...
import {currentlyOpen, minutesSinceSunday, getNextTimeSlot} from '../cmueats/time'
import {getStatusMessage} from "../cmueats/queryLocations"
import { retrieveEvents } from "@/pages/api/query";

export async function getImageURL(buildingCode:string, room:string|null):Promise<string>{
    if(room === null){
        return `/assets/location_images/${simplify(buildingCode)}.jpg`
    }else{
        //check if file exists
        let res = await fetch("/assets/location_images/list_of_files.txt")
        let txt = await res.text()
        return txt.indexOf(room)!= -1 ? `/assets/location_images/${buildingCode}/${simplify(room)}.jpg` : getImageURL(buildingCode, null)
    }
    // "./assets/location_images/GHC/4102.jpg"
}

export async function getAvailabilityData(buildingCode:string, room:string):Promise<any[]|null>{
    const specialdata = await fetch(//Use threads here
        `/api/events`,{ 
        method: 'PATCH',
        headers: {
    ...
    let thisroom = [] //= roomsToTimes[buildingCode+"_"+room]
...
export async function getEatingData(alias:string):Promise<any|null>{
    ...
    let res = await fetch("https://dining.apis.scottylabs.org/locations/")
    let eatingData = await res.json()
	if(eatingData.length == 0) return {}
	eatingData.locations = [eatingData.locations.find((e) => e.name == alias.toUpperCase())]
    if(!eatingData) return {}
        ...
        if(timeSlot){
            let diff = (minutesSinceSunday(timeSlot.end.weekday, timeSlot.end.hour, timeSlot.end.minute) -
            ...
            let nextTimeSlot = getNextTimeSlot(times, now) 
            let diff;
            if(!nextTimeSlot){
                diff = minutesSinceSunday(timeSlot.start.weekday, timeSlot.start.hour, timeSlot.start.minute) -
						minutesSinceSunday(now.weekday, now.hour, now.minute);
					if (diff < 0) {
						diff += WEEK_MINUTES;
					}
...
export async function getBuildingWebsites(buildingCode:string|undefined){
    ...
    let res = await fetch("./assets/buildingWebsites.json")
    // var getFavicons = require('get-website-favicon')
    let buildingWebsites = await res.json()
    let websiteList = buildingWebsites[buildingCode+""]
