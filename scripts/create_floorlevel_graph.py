...
for root, dirs, files in os.walk("../public/json/floor_plan"):
    ...
    for file in files:
        ...
        if "graph" in file_path:
            ...
            if building_code not in placements or str(floor) not in placements[building_code]:
                continue
            ...
            with open(file_path, "r") as graph_file:
                with open(file_path.replace("graph", "outline"), "r") as outline_file:
                    ...
                    for nodeId in graph.keys():
                        ...
                        node['coordinate'] = position_on_map(node["pos"], placement, floor_center)
                        for neighbor in node['neighbors'].keys():
                            ...
                            if "toFloorInfo" in node['neighbors'][neighbor]:
                                ...
                                big_graph[f"{building_code}-{floor}"].add((new_floor, stairs_info["type"]))
                                if new_floor not in big_graph:
                                    big_graph[new_floor] = set()
                                big_graph[new_floor].add((f"{building_code}-{floor}", stairs_info["type"]))
