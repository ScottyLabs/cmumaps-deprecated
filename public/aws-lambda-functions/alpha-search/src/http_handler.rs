...
use search::types::{Document, Documents, SearchIndex};
...
pub(crate) async fn function_handler(event: Request, index: &SearchIndex, documents: &Documents) -> Result<Response<Body>, Error> {
    ...
    let vec_query = search::parse_query(query);
    
    let response_obj = search::search(&vec_query, &index, &documents, None, 100)
